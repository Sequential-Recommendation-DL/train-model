...
def normalize_comparison():
    ...
    sigmoid = 2.0 / (1.0 + np.exp(-x))
    tanh = 2.0 * np.tanh(x / 5.0)

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(x, sigmoid, label=r"$2 \cdot \sigma(x)$", linewidth=2, color="#FF6B6B")
    ax.plot(x, tanh, label=r"$2 \cdot \tanh(x / 5)$", linewidth=2, color="#4ECDC4")

    behaviors = {"pv": 1, "fav": 2, "cart": 3, "buy": 4}
    colors = {"pv": "#95E1D3", "fav": "#FFE66D", "cart": "#FF6B6B", "buy": "#4ECDC4"}
    for name, score in behaviors.items():
        sig_v = 2.0 / (1.0 + np.e**-score)
        tan_v = 2.0 * np.tanh(score / 5.0)
        ...
        ax.annotate(f"  {name}({score})", (score, 1.5), fontsize=9, rotation=90,
                    va="center", ha="left", alpha=0.7)

    ax.axhline(2.0, color="gray", linestyle="--", linewidth=0.5, alpha=0.5)
