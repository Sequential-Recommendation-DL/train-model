...
def negative_sample(df, train_data, all_interactions_df, num_neg: int = 4):
    """
    ...
    print("\n===== NEG SAMPLE DEBUG =====")
    print("user:", pos_users[0])
    print("positive item:", pos_items[0])
    print("negative items:", neg_items[0][:10])
    print("===========================\n")

    X = torch.tensor(
        # np.stack([all_users, all_items, all_brands, all_cats,
        np.stack([all_users, all_items, all_cats,
...
def run_pipeline(
    ...
    sample_seed: int = 42,
    num_neg_train: int = 4,   # negative/positive cho train, ban đầu là 4 để train nhanh
    ...
    if skip_preprocess and all(os.path.exists(p) for p in [processed_train, processed_val, processed_test]):
        ...
        #     print("Brand unique:", df["brand"].nunique(dropna=False))
        #     print(df["brand"].head())

        if "main_category" in df.columns:
            print("Category unique:", df["main_category"].nunique(dropna=False))
    ...
    print("item_idx unique:", train_df["item_idx"].nunique())

    print(
        "mapped unique:",
        train_df["item_idx"]
            .map(train_data.item_map)
            .fillna(0)
            .astype(int)
            .nunique()
    )
    ...
    with torch.no_grad():
        for X, y in test_loader:
            ...
            print("Preds:", probs[:20].cpu().numpy())
            print("Label:", y[:20].cpu().numpy())
            break
    ...
    result_dir = generate_results(
        ...
        train_data.user_map, train_data.item_map,
        device
    )
