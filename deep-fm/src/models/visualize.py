...
#     print("train item unique:", len(train_items))
#     print("test item unique :", len(test_items))

#     unseen = test_items - train_items

#     print("unseen test items:", len(unseen))
#     print("percent unseen:", len(unseen) / len(test_items))
...
#     num_users, num_items, num_cats, num_prices, num_hours, num_dayofweeks  = field_dims
...
#                 print(
#                     f"item={c}, label={lbl}, score={score:.6f}"
#                 )
#             X_list.append([u_idx, c, cat_idx, p_idx, h_idx, d_idx])
#             y_list.append(lbl)
...
#     print("Positives:", sum(all_labels))
#     print("Negatives:", len(all_labels) - sum(all_labels))

#     scores_np = np.array(all_scores)

#     print("Mean score positive:",
#         scores_np[np.array(all_labels)==1].mean())

#     print("Mean score negative:",
#         scores_np[np.array(all_labels)==0].mean())
...
#         model, test_df, train_df, user_map, item_map, device, result_dir
...
def plot_ranking_metrics(history: list[dict], metrics: dict, save_dir: str) -> None:
    """Vẽ biểu đồ HR@10 và NDCG@10"""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    # HR@10 và NDCG@10 chỉ có 1 điểm (test set) nên vẽ bar chart
    ax1.bar(["HR@10"], [metrics.get("HR@10", 0)], color="steelblue")
    ax1.set_ylim(0, 1)
    ax1.set_title("Hit Rate @ 10")
    ax1.set_ylabel("HR@10")
    ax1.axhline(y=0.5, color="gray", linestyle="--", label="Baseline 0.5")
    ax1.legend()
    ...
    ax2.bar(["NDCG@10"], [metrics.get("NDCG@10", 0)], color="seagreen")
    ax2.set_ylim(0, 1)
    ax2.set_title("NDCG @ 10")
    ax2.set_ylabel("NDCG@10")
    ax2.axhline(y=0.3, color="gray", linestyle="--", label="Baseline 0.3")
    ax2.legend()
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "ranking_metrics.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  → Đã lưu: {path}")
...
def generate_results(
    ...
    plot_training_history(history, result_dir)
    plot_confusion_matrix(model, test_loader, device, result_dir)
    plot_ranking_metrics(history, metrics, result_dir)
    plot_roc_curve(model, test_loader, device, result_dir)
    save_metrics(metrics, history, result_dir)
    print(f"  → HR@10: {metrics.get('HR@10', 0):.4f}  NDCG@10: {metrics.get('NDCG@10', 0):.4f}")
